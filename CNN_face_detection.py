...
def deepnn(x):
  ...
  with tf.name_scope('fc2'):
    W_fc2 = weight_variable([256, people])
    b_fc2 = bias_variable([people])
...
def main(_):
  ...
  y_ = tf.placeholder(tf.int64, [None,people])
  ...
  with tf.name_scope('accuracy'):
    #y_conv is the result_label you got from NN,y_ is the label
    correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
    pred_res = correct_prediction#to show the result of comparison
